...
export type Settled = {op: SettledOp; timer: Timer; clear: boolean; id: string; logged?: number};
...
export function settle(now = Date.now()): Settled[] {
 ...
 if (focus) {
  ...
  // Each block carries the `logged` value to record once its change is accepted; nothing moves forward before that.
  for (let i = logged; i < completed; i++) out.push({timer: focus, clear: false, logged: i + 1, id: stableId(`focus|${focus.key}|${focus.day}|${focus.meta?.run ?? focus.startedAt ?? 'p'}|${focus.meta?.work}|${focus.meta?.rest}|block${i}`), op: {type: 'focus', seconds: w, day: focus.day}});
...
// Apply the settled operations through the app's dispatcher: dispatch first, and only an accepted change clears its timer or moves a focus
// session's `logged` forward. A refused change (device storage full, out of bounds) leaves the timer exactly as it was, so the next settle
// retries with the same stable ids; the device queues an id once and the account applies it once, so retries are safe.
export function applySettled(list: Settled[], change: (op: Settled['op'], id: string) => boolean) {
 let accepted = 0;
 for (const item of list) {
  const end = item.op.type === 'focus' && item.op.seconds === 0;
  const ok = end ? true : change(item.op, item.id);
  if (!ok) break;
  accepted++;
  if (item.logged !== undefined) updateTimer(item.timer.key, {logged: item.logged});
  if (item.clear) clearTimer(item.timer.key);
 }
 return accepted;
}
