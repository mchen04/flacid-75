...
export function checkBounds(op: Operation): string | null {
 ...
 switch (op.type) {
  ...
  case 'containers': return op.containers.length >= 1 && op.containers.length <= limits.containers && op.containers.every(c => typeof c.id === 'string' && c.id.length >= 1 && c.id.length <= 40 && text(c.name, limits.containerName) && inRange(c.ml, limits.containerMl)) ? null : out;
  case 'profile': return inRange(op.stats.height, limits.height) && inRange(op.stats.weight, limits.weight) && Number.isInteger(op.stats.age) && inRange(op.stats.age, limits.age) ? null : out;
