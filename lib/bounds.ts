...
import type {Operation, MealRecord} from './domain';
...
export function mealTextError(meal: Pick<MealRecord, 'description' | 'items'>): string | null {
 if (meal.description !== undefined && !textOptional(meal.description, limits.mealDescription)) return 'The meal description contains text that cannot be saved. Edit the description.';
 if (meal.items?.some(i => !textOptional(i.name, 120) || (i.match !== undefined && !textOptional(i.match, 200)))) return 'A meal item contains text that cannot be saved. Edit its name or remove the item.';
 return null;
}
...
export function checkBounds(op: Operation): string | null {
 ...
 switch (op.type) {
  ...
  case 'meal': return mealTextError(op) ?? (isUuid(op.mealId) && inRange(op.calories, limits.calories) && inRange(op.protein, limits.protein) && (op.items === undefined || (op.items.length <= limits.mealItems && op.items.every(i => inRange(i.grams, [0, 5000]) && inRange(i.calories, limits.calories) && inRange(i.protein, limits.protein) && ['usda', 'estimate'].includes(i.source) && (i.fdcId === undefined || (Number.isSafeInteger(i.fdcId) && i.fdcId >= 0))))) ? null : 'A meal must be 0–10,000 kcal and 0–1,000 g protein.');
