...
// Symbols (¾ ½ ¼) are not word characters, so they are matched without word boundaries.
...
// A mixed number ("two and a half glasses", "1 and a quarter Stanleys", "1½ glasses") is one pour of that many containers.
export function mixedOf(text: string): number | null {
 // The container's name has already been replaced by "it" ("a glass and a half" reads "a it and a half").
 const t = text.toLowerCase(); const m = t.match(/\b(\d+|a|an|one|two|three|four|five|six)\s+(?:it\s+)?and\s+(?:a\s+)?(half|quarter|third|three[- ]quarters?|two[- ]thirds?)\b/) ?? t.match(/\b(\d+)\s*(½|¼|¾)/);
 if (!m) return null; const whole = words[m[1]] ?? Number(m[1]); const part = fractionOf(m[2]); if (!Number.isFinite(whole) || whole < 1 || whole > 20 || part === null) return null; return whole + part;
}
...
export function countOf(text: string): number | null {
 // A slash fraction ("3/4", "1/2") is a share; neither of its numbers is a count.
 const t = text.toLowerCase().replace(/\d+\s*\/\s*\d+/g, 'share');
 ...
 // "twice" and "thrice" count on their own ("half a Stanley twice").
 if (/\btwice\b/.test(t)) return 2; if (/\bthrice\b/.test(t)) return 3;
...
export function describe(container: Container, fraction: number, count: number) {
 ...
 if (fraction > 1) {const w = Math.floor(fraction), f = fraction - w; const part = Math.abs(f - .5) < .01 ? '½' : Math.abs(f - .25) < .01 ? '¼' : Math.abs(f - .75) < .01 ? '¾' : Math.abs(f - 1 / 3) < .01 ? '⅓' : Math.abs(f - 2 / 3) < .01 ? '⅔' : ''; const n = `${w}${part}`; return count === 1 ? `${n} ${plural}` : `${count} × ${n} ${plural}`;}
...
export function parsePhrase(text: string, containers: Container[], fallback: Container): Parsed {
 ...
 const mixed = mixedOf(rest); const fraction = mixed ?? fractionOf(rest); const count = mixed !== null ? 1 : countOf(rest);
