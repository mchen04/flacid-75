...
export function finishTimer(key: string, build: (seconds: number, day: string, id: string) => boolean, now = Date.now()) {
 const timer = getTimer(key); if (!timer) return false;
 const ok = build(cappedSeconds(elapsedSeconds(timer, now)), timer.day, stableId(`finish|${timer.run ?? JSON.stringify(timer)}`));
