...
export function Home({hour, stumbled}: {hour: number; stumbled: boolean}) {
 const {state, today, dayKey, selected, day, done, pulse, change, navigate, open, bump} = useApp();
 ...
 function toggle(h: Habit) {const before = done[h]; if (change({type: 'check', habit: h, value: !before}, dayKey)) bump(h, h === 'walk' ? 2000 : 1400);}
 ...
 return <section className="home-view">
  <button className={`hero ${done.walk ? 'is-done' : ''} ${pulse.walk ? 'moving' : ''} ${complete ? 'is-complete' : ''}`} aria-label={`Open walk. ${count} of ${habits.length} habits done today`} onClick={() => navigate('walk')}>
  ...
  <div className="rows">
   ...
   <div className={`row water ${done.water ? 'is-done' : ''} ${pulse.water ? 'pouring' : ''}`}>
    <button className="row-open" onClick={() => navigate('water')} aria-label="Open water"><span className="row-art glass-art"><Glass level={day.water / day.targets.water} pouring={!!pulse.water}/></span><span className="row-copy"><span className="row-title">Water</span><span className="row-status">{litres(day.water)} of {litres(day.targets.water)} L · {Math.round(day.water / 250)} of {Math.round(day.targets.water / 250)} glasses</span><Meter label="" value={day.water} max={day.targets.water} unit="" done={done.water} display=""/></span><span className="row-chevron"><Icon name="arrow" size={18}/></span></button>
    <button className="row-action is-add" aria-label="Add a glass of water" onClick={pour}><Icon name="plus" size={16}/>Glass</button>
   </div>
   <div className={`row food ${done.calories && done.protein ? 'is-done' : ''} ${pulse.meal ? 'eating' : ''}`}>
    <button className="row-open" onClick={() => navigate('food')} aria-label="Open food"><span className="row-art bowl-art"><Bowl full={food.calories > 0} eating={!!pulse.meal} level={food.calories / Math.max(1, day.targets.calorieMax)}/></span><span className="row-copy"><span className="row-title">Food</span><span className="row-status">{format(food.calories)} kcal · {format(food.protein)}/{format(day.targets.protein)} g protein</span><Meter label="" value={food.calories} min={day.targets.calorieMin} max={day.targets.calorieMax} unit="" done={done.calories} display=""/></span><span className="row-chevron"><Icon name="arrow" size={18}/></span></button>
    <button className="row-action is-add" aria-label="Log a meal" onClick={() => open('meal')}><Icon name="plus" size={16}/>Meal</button>
   </div>
   <Row mark="rest" title="Rest" status={day.rest ? 'Resting today · the streak stays' : `${rests} of 2 left this week`} done={day.rest} onOpen={() => navigate('rest')} tone="rest" action={<button className="row-action" aria-label={day.rest ? 'Undo rest' : 'Rest today'} aria-pressed={day.rest} onClick={restTap}>{day.rest ? <><Icon name="check" size={16}/>Undo</> : 'Rest'}</button>} pulse={pulse.rest ? 'dozing' : ''}/>
  </div>
