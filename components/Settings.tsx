...
export function Setup({initial, onSave}: {initial?: Stats & {overrides?: Partial<Targets>; units?: Units}; onSave: (s: Stats, o: Partial<Targets>, u: Units) => void}) {
 ...
 return <form className="setup-form" onSubmit={e => {e.preventDefault(); const f = new FormData(e.currentTarget); const activity = Number(f.get('activity')); const goal = f.get('goal');
  ...
  const untouchedHeight = !!shown && (units.height === 'cm' ? String(f.get('cm')) === shown.cm : String(f.get('feet')) === shown.feet && String(f.get('inches')) === shown.inches) && (initial?.units?.height ?? defaultUnits.height) === units.height;
  const untouchedWeight = !!shown && String(f.get('weight')) === shown.weight && (initial?.units?.weight ?? defaultUnits.weight) === units.weight;
