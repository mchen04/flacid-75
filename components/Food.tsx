...
export function Food() {
 ...
 return <section className="activity">
  ...
  <p className="fine-print">Calories count when the day lands inside the range; protein when it reaches the target. Estimates come from your words or photo and are always yours to correct. The meal you describe or photograph goes to a free third-party model for the estimate; your logs sync to your private account and nowhere else.</p>
 </section>;
}
...
export function Meal({photo, initial, count, day, today, onSave, onPhotoConsumed, onCamera, onList}: {photo: File | null; initial?: {calories: number; protein: number}; count: number; day: string; today: string; onPhotoConsumed: () => void; onCamera: () => void; onList: () => void; onSave: (calories: number, protein: number) => void}) {
 ...
 const addLabel = day === today ? 'Add to today' : `Add to ${shortDate(day)}`;
 // One save per sheet: a double tap on "Add" cannot log the same meal twice. Totals are bounded to what the account accepts.
 function save(c: number, p: number) {if (saved.current) return; if (c < limits.calories[0] || c > limits.calories[1] || p < limits.protein[0] || p > limits.protein[1]) {setError(`A meal must be 0–${limits.calories[1].toLocaleString()} kcal and 0–${limits.protein[1].toLocaleString()} g protein.`); return;} saved.current = true; onSave(c, p);}
 const sum = items?.reduce((a, i) => ({calories: a.calories + i.calories, protein: a.protein + i.protein}), {calories: 0, protein: 0});
 if (items && sum) return <div className="found"><ul className="items">{items.map((item, i) => <li key={i}><div><strong>{item.name}</strong><small>{item.grams} g · {item.source === 'usda' ? 'USDA · ' + item.match : 'estimate'}</small></div><label>Calories · kcal<input type="number" inputMode="decimal" min="0" max="10000" value={item.calories} onChange={e => setItems(items.map((it, j) => j === i ? {...it, calories: clamp(Number(e.target.value), limits.calories)} : it))}/></label><label>Protein · g<input type="number" inputMode="decimal" min="0" max="1000" step="0.1" value={item.protein} onChange={e => setItems(items.map((it, j) => j === i ? {...it, protein: clamp(Number(e.target.value), limits.protein)} : it))}/></label></li>)}</ul>
  <p className="sum"><strong>{format(sum.calories)} kcal · {Math.round(sum.protein * 10) / 10} g protein</strong> estimate</p>
  {error && <p className="form-error" role="alert">{error}</p>}
  <button className="primary" onClick={() => save(Math.round(sum.calories), Math.round(sum.protein * 10) / 10)}>{addLabel}</button><button className="secondary" onClick={() => {setItems(null);}}>Not this</button></div>;
