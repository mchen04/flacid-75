...
const state=await(await fetch(base+'/api/state',{headers:{cookie}})).json();assert.ok('profile' in state&&'days' in state);
...
const result={url:base,unauthenticated:negative,authenticatedGate:login.status,crossOriginWrite:csrf.status,accountConfigured:state.profile!==null,deployedClientMatchesVerifiedBuild:true};
