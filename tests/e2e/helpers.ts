...
export async function open(page:Page,state:State,{hash='',go=true,unlocked=true}:{hash?:string;go?:boolean;unlocked?:boolean}={}){
 const box={state,ops:[] as Operation[],rejected:[] as {id:string;reason:string}[]};
 await page.route('**/api/state',r=>r.fulfill({json:box.state}));
 // Exactly what the server does: validate each change with the real schema, apply the valid ones, report the rest as rejected.
 await page.route('**/api/sync',r=>{const raw=r.request().postDataJSON();const batch=validateBatch(raw);if(!batch)return r.fulfill({status:400,json:{error:'A batch must be 1 to 100 changes.'}});const rejected:{id:string;reason:string}[]=[...batch.invalid];for(const o of batch.valid as Operation[]){try{box.state=apply(box.state,o);box.ops.push(o);}catch(e){rejected.push({id:o.id,reason:e instanceof Error?e.message:'no'});}}box.rejected.push(...rejected);return r.fulfill({json:{state:box.state,accepted:batch.valid.map(o=>o.id),rejected}});});
