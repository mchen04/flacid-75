...
export async function mock(page:Page,box:Box){
 ...
 await page.route('**/api/sync',r=>{try{if(new TextEncoder().encode(r.request().postData()??'').length>100000)return r.fulfill({status:413,json:{error:'Too many changes at once.'}});const raw=r.request().postDataJSON();const batch=validateBatch(raw);if(!batch)return r.fulfill({status:400,json:{error:'A batch must be 1 to 100 changes.'}});const rejected:{id:string;reason:string}[]=[...batch.invalid];for(const o of batch.valid as Operation[]){if(box.seen.has(o.id))continue;try{box.state=apply(box.state,o);box.ops.push(o);box.seen.add(o.id);}catch(e){rejected.push({id:o.id,reason:e instanceof Error?e.message:'no'});}}box.rejected.push(...rejected);return r.fulfill({json:{state:box.state,accepted:batch.valid.map(o=>o.id),rejected,...receipt()}});}catch(e){console.error('mock account failed:',e);return r.fulfill({status:500,json:{error:'mock account failed: '+(e instanceof Error?e.message:String(e))}});}});
}
