...
test('offline optimistic writes, reconnect deduplication and manual fallback when the estimate fails',async({page,context})=>{
 ...
 await page.getByRole('button',{name:'Log workout'}).click();await expect(page.getByRole('button',{name:'Undo workout'})).toHaveAttribute('aria-pressed','true');await page.getByRole('button',{name:'Add a glass of water'}).click();await expect(page.getByText('1 of 8 · 0.25 L')).toBeVisible();
