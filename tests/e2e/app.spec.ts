...
test('onboarding in pounds and feet-inches, all seven one-tap habits from the dashboard, celebration, correction and calendar',async({page})=>{
 ...
 await page.goto('/');await expect(page.getByRole('heading',{name:'My Wellness'})).toBeVisible();await page.getByLabel('Passphrase').fill(virtual?'test-only':secret);await page.getByRole('button',{name:'Open'}).click();
 await expect(page.getByRole('heading',{name:'Welcome to My Wellness.'})).toBeVisible();
 ...
 const water=page.getByRole('button',{name:'Add a glass of water'});for(let i=0;i<8;i++)await water.click();await expect(page.getByText('2 of 2 L · 8 of 8 glasses')).toBeVisible();
 ...
 await page.getByLabel('What did you eat?').fill('two eggs and toast');await page.getByRole('button',{name:'Look it up'}).click();await page.getByRole('button',{name:'Add to today'}).click();await expect(page.getByText('294 kcal · 15/105 g protein')).toBeVisible();
 ...
 await expect(page.getByRole('heading',{name:'Every one.'})).toBeVisible();await expect(page.locator('.hero')).toHaveClass(/is-complete/);await expect(page.locator('.hero .petal').first()).toBeAttached();await expect(page.getByText('+100 points')).toBeVisible();await page.screenshot({path:`evidence/my-wellness/celebration-${test.info().project.name}.png`});
 // Correct a meal on the food page, in place.
 await page.getByRole('button',{name:'Open food'}).click();await page.getByRole('button',{name:'Correct meal 1'}).click();await page.getByLabel('Calories · kcal').fill('300');await page.getByRole('button',{name:'Save',exact:true}).click();await expect(page.getByText('1,900 kcal')).toBeVisible();
...
test('offline optimistic writes, reconnect deduplication and manual fallback when the estimate fails',async({page,context})=>{
 ...
 await page.getByRole('button',{name:'Log workout'}).click();await expect(page.getByRole('button',{name:'Undo workout'})).toHaveAttribute('aria-pressed','true');await page.getByRole('button',{name:'Add a glass of water'}).click();await expect(page.getByText('0.25 of 2 L')).toBeVisible();
 ...
 await page.route('**/api/estimate',r=>r.fulfill({status:503,json:{error:'That could not be looked up right now.'}}));await page.getByRole('button',{name:'Log a meal'}).click();await page.getByLabel('What did you eat?').fill('two eggs');await page.getByRole('button',{name:'Look it up'}).click();await expect(page.locator('.form-error')).toContainText('could not be looked up');await page.getByRole('button',{name:'Enter numbers'}).click();await page.getByLabel('Calories · kcal').fill('160');await page.getByLabel('Protein · g',{exact:true}).fill('12');await page.getByRole('button',{name:'Add to today'}).click();await expect(page.getByText('160 kcal · 12/105 g protein')).toBeVisible();
