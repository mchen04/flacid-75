...
test('a tap changes state in under 100 ms and shifts no layout, with every habit animated distinctly',async({page})=>{
 ...
 for(const habit of ['workout','abs','walk','floss'])await expect(page.getByRole('button',{name:`Undo ${habit}`})).toHaveAttribute('aria-pressed','true');
 ...
 await page.getByRole('button',{name:'Undo workout'}).click();await expect(page.getByRole('button',{name:'Log workout'})).toHaveAttribute('aria-pressed','false');
