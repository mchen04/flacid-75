...
const rects=(page:Page)=>page.evaluate(()=>Object.fromEntries([...document.querySelectorAll<HTMLElement>('.track,.card,.tile,.bottom-nav,.greeting,.home-footer,.topbar')].map((el,i)=>{const r=el.getBoundingClientRect();return [el.className.split(' ')[0]+i,[Math.round(r.left),Math.round(r.top),Math.round(r.width),Math.round(r.height)]];})));
test('a tap changes state in under 100 ms and shifts no layout, with every habit animated distinctly',async({page})=>{
 ...
 for(const habit of ['Workout','Abs','Walk','Floss','Water']){
  ...
  await expect(button).toHaveClass(new RegExp({Workout:'lifting',Abs:'crunching',Walk:'moving',Floss:'shining',Water:'pouring'}[habit]!));
...
test('walking moves Pip along the track and eating shows Pip with the meal',async({page})=>{
 await open(page,seed());
 ...
 await expect(page.locator('.food')).toHaveClass(/eating/);await expect(page.locator('.food .pip-eat')).toBeVisible();await page.screenshot({path:`evidence/pip-eats-${test.info().project.name}.png`});await expect(page.getByText('107 kcal')).toBeVisible();
});
