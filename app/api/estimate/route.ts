...
export async function POST(req:Request){const denied=await guard(req);if(denied)return denied;
 try{
  ...
  if(text.length>1000||image&&(!/^[A-Za-z0-9+/]+=*$/.test(image)||image.length>2000000)||!text&&!image)return json({error:'Describe the meal or add a clear photo.'},400);
  const input={text,image};body.image=undefined;body.text=undefined;
  return json(await estimateMeal(input,req.signal));
 }catch(error){if(error instanceof NotFood)return json({error:'That does not look like food.'},422);return json({error:'Pip could not look that up right now.'},503);}
}
