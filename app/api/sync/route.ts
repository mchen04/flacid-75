...
export async function POST(req:Request){const denied=await guard(req);if(denied)return denied;try{if(Number(req.headers.get('content-length'))>100000)return json({error:'Too many changes at once.'},413);
 ...
 const result=batch.valid.length?await sync(batch.valid):await (async()=>{const {applied,revision,...state}=await readState();return {state,accepted:[] as string[],rejected:[] as {id:string;reason:string}[],applied,revision};})();
